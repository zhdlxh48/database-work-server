const express = require("express");
const mysql = require("mysql");

const router = express.Router();

const db = mysql.createConnection({
  host: "svc.sel4.cloudtype.app",
  port: 30687,
  user: "testuser",
  password: "testepassword",
  database: "test-db",
});

router.get("/", (req, res) => {
  console.log(req.query);
  db.query(
    // "SELECT item_id, item_type, item_name FROM items where item_id in (SELECT item_id FROM inventories where player_id = ?)",
    `SELECT itm.item_id, item_type, item_name, COUNT(itm.item_id) AS count FROM inventories inv 
        LEFT JOIN items itm ON inv.item_id = itm.item_id where player_id = ? group by item_id`,
    [req.query.id],
    (err, rows) => {
      if (err) {
        console.error(err);
        res
          .status(400)
          .send(JSON.stringify({ result: "FAIL", message: err.sqlMessage }));
        return;
      }

      console.log(rows);

      res.send(JSON.stringify({ result: "OK", message: rows }));
      return;
    }
  );
});

router.post("/", (req, res) => {
  console.log(req.body);
  db.query(
    "INSERT INTO inventories (player_id, item_id) VALUES (?, ?)",
    [req.body.player_id, req.body.item_id],
    (err, rows) => {
      if (err) {
        console.error(err);
        res
          .status(400)
          .send(JSON.stringify({ result: "FAIL", message: err.sqlMessage }));
        return;
      }

      console.log(rows);

      res.send(JSON.stringify({ result: "OK", message: rows }));
      return;
    }
  );
});

module.exports = router;
